 ...
 */
public class CompanyExpiryFragment extends Fragment implements View.OnClickListener{
    ...
    public class LoadExpiryListInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = httpOperations.doCMPY_DOC_EXPIRY_LIST(mId, mAuthorization,mStart);
            return result;
        }
            ...
            try {
                ...
                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        ...
                        JSONArray feedArray = jsonObj.getJSONArray("company_document_expiring_date_list");
                        for (int i = 0; i < feedArray.length(); i++) {
                            ...
                            if((!feedObj.getString("company_name").equals(""))
                                ...
                                item.set_name(StringUtils.capitalize(feedObj.getString("company_name")
                                        .toLowerCase().trim()));
                                JSONArray exArray = feedObj.getJSONArray("Document Expiring Dates");
                                try {
                                    JSONArray docArray = exArray.getJSONArray(0);
                                    String[] docdata = new String[docArray.length()];
                                    String[] docdatadate = new String[docArray.length()];
                                    for (int j = 0; j < docArray.length(); j++) {
                                        JSONObject expiryObj = (JSONObject) docArray.get(j);

                                        if(expiryObj.getString("doc_title").equals("")){
                                            docdata[j] = "None";
                                        } else {
                                            docdata[j] = expiryObj.getString("doc_title");
                                        }

                                        if(expiryObj.getString("doc_end_date").equals("")){
                                            docdatadate[j] = "None";
                                        } else {
                                            docdatadate[j] = expiryObj.getString("doc_end_date");
                                        }
                                    }
                        ...
                        start = dataItem.size();
                        mExpiryRecyclerAdapter = new ExpiryRecyclerAdapter(getActivity(), dataItem,"company");
