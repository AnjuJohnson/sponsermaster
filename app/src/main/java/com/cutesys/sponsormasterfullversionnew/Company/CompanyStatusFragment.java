...
public class CompanyStatusFragment  extends Fragment implements View.OnClickListener{
    ...
    public class LoadCompyStatusInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = httpOperations.doCMPY_DOC_STATUS_LIST(mId, mAuthorization,mStart);
            return result;
        }
            ...
            try {
                ...
                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        ...
                        JSONArray feedArray = jsonObj.getJSONArray("company_doc_status_list");
                        try {
                            if (feedArray.length() > 0) {
                                for (int i = 0; i < feedArray.length(); i++) {
                                    ...
                                    if((!feedObj.getString("company_name").equals(""))
                                        ...
                                        item.set_employee_code(feedObj.getString("company_code"));
                                        ...
                                        item.set_address(statusObj.getString("document_staus_municipal_baladiya"));
                                        item.set_code(statusObj.getString("document_staus_tax_card"));
                                        item.set_model(statusObj.getString("document_staus_computer_card"));
                                        item.set_manufacture(statusObj.getString("document_staus_commercial_registration"));
