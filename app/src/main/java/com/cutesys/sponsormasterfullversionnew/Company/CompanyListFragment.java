...
System.out.println("?????????????????????"+Data_Item.get(0).get("home_cpy_notification"));
    ...
    public class LoadCompyListInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = httpOperations.doCompanyList(mId, mAuthorization,mStart);
            return result;
        }
                    ...
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        ...
                        for (int i = 0; i < feedArray.length(); i++) {
                            ...
                            if((!feedObj.getString("company_name").equals(""))
                                ...
                                item.set_code(feedObj.getString("company_code"));
                                ...
                                if(feedObj.getString("company_phone").trim().equals("")){
                                    item.set_phone("None");
                                }else {
                                    item.set_phone(feedObj.getString("company_phone"));
                                }
                                if (feedObj.getString("company_logo").equals("")) {
                                    item.setImage("false");
                                } else {
                                    item.setImage(feedObj.getString("company_logo").trim());
                                }
                                dataItem.add(item);
                            }
                        }
    ...
    public class LoadallData extends AsyncTask<Void, StringBuilder, StringBuilder> {
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            HttpOperations httpOperations = new HttpOperations(getActivity());
            StringBuilder result = httpOperations.doCompanyFullList(mId, mAuthorization,mStart);
            return result;
        }
    ...
    public void loadToolBarSearch() {
        ...
        edtToolSearch.setHint("Search Company");
