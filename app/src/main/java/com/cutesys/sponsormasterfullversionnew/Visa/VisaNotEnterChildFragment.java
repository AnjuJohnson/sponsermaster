...
public class VisaNotEnterChildFragment extends Fragment implements View.OnClickListener{
    ...
    private void InitGetData(boolean temp){
        ...
        if(mConfig.isOnline(getActivity())){
            LoadVisaNotenterInitiate mLoadVisaNotenterInitiate = new LoadVisaNotenterInitiate(sPreferences.getString("ID", ""),
                    sPreferences.getString("AUTHORIZATION", ""), temp, String.valueOf(start));
            mLoadVisaNotenterInitiate.execute((Void) null);
        }else {
    ...
    public class LoadVisaNotenterInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {

        private String mId, mAuthorization, mStart;
        private boolean mStatus;

        LoadVisaNotenterInitiate(String id, String authorization, boolean status, String Start) {
            mId = id;
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder  result = httpOperations.doVisaNotEnteredList(mId, mAuthorization,mStart);
            return result;
        }
            ...
            try {
                ...
                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        ...
                        for (int i = 0; i < feedArray.length(); i++) {
                            ...
                            if ((!feedObj.getString("visa_applicant_name").equals(""))
                                ...
                                if(feedObj.getString("visa_number").trim().equals("")){
                                    ...
                                    item.set_model(feedObj.getString("visa_number"));
                                }
                                if(feedObj.getString("visa_entry_date").trim().equals("")){
                                    item.set_email("None");
                                }else {
                                    item.set_email(feedObj.getString("visa_entry_date"));
                                }
                                if(feedObj.getString("visa_expiry_date").trim().equals("")){
                                    item.set_phone("None");
                                }else {
                                    item.set_phone(feedObj.getString("visa_expiry_date"));
                                }
                        ...
                        start = dataItem.size();
                        mVisaListAdapter = new VisaListAdapter(getActivity(), dataItem,"notenter");
