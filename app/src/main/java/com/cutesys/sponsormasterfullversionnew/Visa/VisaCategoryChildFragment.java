...
import com.cutesys.sponsormasterfullversionnew.ADDModule.AddVisaCategoryActivity;
 ...
 */
public class VisaCategoryChildFragment extends Fragment implements View.OnClickListener{
    ...
    private void InitGetData(boolean temp){
        ...
        if(mConfig.isOnline(getActivity())){
            LoadCategoryInitiate mLoadCategoryInitiate = new LoadCategoryInitiate(sPreferences.getString("ID", ""),
    ...
    public void onClick(View view) {
        ...
        switch (buttonId){
            ...
            case R.id.empty_label_retry:
                ...
                break;
            case R.id.mAdd:
                Intent intent = new Intent(getActivity(),AddVisaCategoryActivity.class);
    ...
    public class LoadCategoryInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {

        private String mId, mAuthorization, mStart;
        private boolean mStatus;

        LoadCategoryInitiate(String id, String authorization, boolean status, String Start) {
            mId = id;
                ...
                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        ...
                        for (int i = 0; i < feedArray.length(); i++) {
                            ...
                            if ((!feedObj.getString("visa_category").equals(""))
                                    && (!feedObj.getString("visa_category").equals("null"))) {

                                if(feedObj.getString("visa_category").trim().equals("")){
                                    item.set_id("None");
                                }else {
                                    item.set_id(StringUtils.capitalize(feedObj.getString("visa_category")
                                            .toLowerCase().trim()));
                                }
                        ...
                        start = dataItem.size();
                        mVisaCatergoryAdapter = new VisaCaty_SponsAdapter(getActivity(), dataItem,"category");
