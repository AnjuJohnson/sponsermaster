...
import com.cutesys.sponsormasterfullversionnew.Adapterclasses.VisaPaymentAdapter;
...
public class VisaPaymentFragment extends Fragment implements View.OnClickListener{
    ...
    private VisaPaymentAdapter mVisaPaymentAdapter;
    ...
    private void InitGetData(boolean temp){
        ...
        if(mConfig.isOnline(getActivity())){
            LoadVisaPaymentInitiate mLoadVisaPaymentInitiate = new LoadVisaPaymentInitiate(sPreferences.getString("ID", ""),
                    sPreferences.getString("AUTHORIZATION", ""), temp, String.valueOf(start));
            mLoadVisaPaymentInitiate.execute((Void) null);
        }else {
    ...
    public class LoadVisaPaymentInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {

        private String mId, mAuthorization, mStart;
        private boolean mStatus;

        LoadVisaPaymentInitiate(String id, String authorization, boolean status, String Start) {
            mId = id;
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = httpOperations.doVisaPaymentList(mId, mAuthorization,mStart);
            return result;
        }
                    ...
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        ...
                        for (int i = 0; i < feedArray.length(); i++) {
                            ...
                            if ((!feedObj.getString("visa_applicant_name").equals(""))
                                ...
                                if(feedObj.getString("payment_month").trim().equals("")){
                                    item.set_month("None");
                                }else {
                                    item.set_month(feedObj.getString("payment_month"));
                                }
                                if(feedObj.getString("payment_year").trim().equals("")){
                                    item.set_year("None");
                                }else {
                                    item.set_year(feedObj.getString("payment_year"));
                                }

                                if(feedObj.getString("visa_type").trim().equals("")){
                                    item.set_address("None");
                                }else {
                                    item.set_address(feedObj.getString("visa_type"));
                                }
                                if(feedObj.getString("visa_category").trim().equals("")){
                                    item.set_email("None");
                                }else {
                                    item.set_email(feedObj.getString("visa_category"));
                                }
                                if(feedObj.getString("visa_client_name").trim().equals("")){
                                    item.set_manufacture("None");
                                }else {
                                    item.set_manufacture(feedObj.getString("visa_client_name"));
                                }
                                if(feedObj.getString("visa_sponsor").trim().equals("")){
                                    item.set_phone("None");
                                }else {
                                    item.set_phone(feedObj.getString("visa_sponsor"));
                                }
                                if(feedObj.getString("visa_total_fee").trim().equals("")){
                                    item.set_model("None");
                                }else {
                                    item.set_model(feedObj.getString("visa_total_fee"));
                                }
                                if(feedObj.getString("visa_bank_fee").trim().equals("")){
                                    item.set_bank_fee("None");
                                }else {
                                    item.set_bank_fee(feedObj.getString("visa_bank_fee"));
                                }
                                if(feedObj.getString("visa_sponsor_fee").trim().equals("")){
                                    item.set_sponsor_fee("None");
                                }else {
                                    item.set_sponsor_fee(feedObj.getString("visa_sponsor_fee"));
                                }
                                if(feedObj.getString("visa_company_fee").trim().equals("")){
                                    item.set_company_fee("None");
                                }else {
                                    item.set_company_fee(feedObj.getString("visa_company_fee"));
                                }
                                if(feedObj.getString("visa_advance_fee").trim().equals("")){
                                    item.set_advance_fee("None");
                                }else {
                                    item.set_advance_fee(feedObj.getString("visa_advance_fee"));
                                }
                                if(feedObj.getString("visa_balance_fee").trim().equals("")){
                                    item.set_balance_fee("None");
                                }else {
                                    item.set_balance_fee(feedObj.getString("visa_balance_fee"));
                                }
                        ...
                        start = dataItem.size();
                        mVisaPaymentAdapter = new VisaPaymentAdapter(getActivity(), dataItem);
                        mrecyclerview.setAdapter(mVisaPaymentAdapter);
                    }else {
