...
public class VisaIssuedChildFragment extends Fragment implements View.OnClickListener{
    ...
    private VisaIssuedAdapter mVisaIssuedAdapter;
    ...
    private void InitIdView(View rootView){
        ...
        mTitle.setText("Issued Visa List");
        InitGetData(false);
    }
    ...
    private void InitGetData(boolean temp){
        ...
        if(mConfig.isOnline(getActivity())){
            LoadVisaIssuedInitiate mLoadVisaIssuedInitiate = new LoadVisaIssuedInitiate(sPreferences.getString("ID", ""),
                    sPreferences.getString("AUTHORIZATION", ""), temp, String.valueOf(start));
            mLoadVisaIssuedInitiate.execute((Void) null);
        }else {
    ...
    public class LoadVisaIssuedInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {

        private String mId, mAuthorization, mStart;
        private boolean mStatus;

        LoadVisaIssuedInitiate(String id, String authorization, boolean status, String Start) {
            mId = id;
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = httpOperations.doVisaIssuedList(mId, mAuthorization,mStart);
            return result;
        }
                    ...
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        ...
                        for (int i = 0; i < feedArray.length(); i++) {
                            ...
                            if ((!feedObj.getString("visa_applicant_name").equals(""))
                                ...
                                if(feedObj.getString("visa_number").trim().equals("")){
                                    item.set_visanum("None");
                                }else {
                                    item.set_visanum(feedObj.getString("visa_number"));
                                }
                                if(feedObj.getString("visa_ref_no").trim().equals("")){
                                    item.set_refno("None");
                                }else {
                                    item.set_refno(feedObj.getString("visa_ref_no"));
                                }
                                if(feedObj.getString("visa_amount_balance").trim().equals("")){
                                    item.set_amount("None");
                                }else {
                                    item.set_amount(feedObj.getString("visa_amount_balance"));
                                }

                                if(feedObj.getString("visa_entry_date").trim().equals("")){
                                    item.set_address("None");
                                }else {
                                    item.set_address(feedObj.getString("visa_entry_date"));
                                }
                                if(feedObj.getString("visa_expiry_date").trim().equals("")){
                                    item.set_expiry("None");
                                }else {
                                    item.set_expiry(feedObj.getString("visa_expiry_date"));
                                }
                        ...
                        start = dataItem.size();
                        mVisaIssuedAdapter = new VisaIssuedAdapter(getActivity(), dataItem,"issued");
                        mrecyclerview.setAdapter(mVisaIssuedAdapter);
                    }else {
