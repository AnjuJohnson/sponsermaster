...
import com.cutesys.sponsormasterfullversionnew.ADDModule.AddExpensesTypeActivity;
...
public class ExpensesTypeFragment extends Fragment implements View.OnClickListener{
    ...
    private void InitIdView(View rootView){
        ...
        mTitle.setText("Expense Type List");
        msubtext.setText("Expense Type");
        InitGetData(false);
    }
    ...
    private void InitGetData(boolean temp){
        ...
        if(mConfig.isOnline(getActivity())){
            LoadExpenseTypeInitiate mLoadExpenseTypeInitiate = new LoadExpenseTypeInitiate(sPreferences.getString("ID", ""),
                    sPreferences.getString("AUTHORIZATION", ""), temp, String.valueOf(start));
            mLoadExpenseTypeInitiate.execute((Void) null);
        }else {
    ...
    public void onClick(View view) {
        ...
        switch (buttonId){
            ...
            case R.id.mAdd:
                Intent intent = new Intent(getActivity(),AddExpensesTypeActivity.class);
    ...
    public class LoadExpenseTypeInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {

        private String mId, mAuthorization, mStart;
        private boolean mStatus;

        LoadExpenseTypeInitiate(String id, String authorization, boolean status, String Start) {
            mId = id;
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = httpOperations.doExpensesTypeList(mId, mAuthorization,mStart);
            return result;
        }
            ...
            try {
                ...
                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        ...
                        JSONArray feedArray = jsonObj.getJSONArray("vehicle_expense_list");
                        for (int i = 0; i < feedArray.length(); i++) {

                            ListItem item = new ListItem();
                            JSONObject feedObj = (JSONObject) feedArray.get(i);

                            if((!feedObj.getString("vehicle_expense").equals(""))
                                    && (!feedObj.getString("vehicle_expense").equals("null"))) {

                                item.set_id(feedObj.getString("vehicle_expense_id"));
                                item.set_name(StringUtils.capitalize(feedObj.getString("vehicle_expense")
                                        .toLowerCase().trim()));
                                dataItem.add(item);
                            }
                        }
