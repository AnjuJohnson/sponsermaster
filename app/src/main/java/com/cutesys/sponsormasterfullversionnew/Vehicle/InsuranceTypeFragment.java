...
import com.cutesys.sponsormasterfullversionnew.ADDModule.AddInsuranceTypeActivity;
...
public class InsuranceTypeFragment extends Fragment implements View.OnClickListener{
    ...
    private void InitIdView(View rootView){
        ...
        mTitle.setText("Insurance Type List");
        msubtext.setText("Insurance Type");
        InitGetData(false);
    }
    ...
    private void InitGetData(boolean temp){
        ...
        if(mConfig.isOnline(getActivity())){
            LoadInsuranceTypeInitiate mLoadInsuranceTypeInitiate = new LoadInsuranceTypeInitiate(sPreferences.getString("ID", ""),
                    sPreferences.getString("AUTHORIZATION", ""), temp, String.valueOf(start));
            mLoadInsuranceTypeInitiate.execute((Void) null);
        }else {
    ...
    public void onClick(View view) {
        ...
        switch (buttonId){
            ...
            case R.id.mAdd:
                Intent intent = new Intent(getActivity(),AddInsuranceTypeActivity.class);
    ...
    public class LoadInsuranceTypeInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {

        private String mId, mAuthorization, mStart;
        private boolean mStatus;

        LoadInsuranceTypeInitiate(String id, String authorization, boolean status, String Start) {
            mId = id;
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = httpOperations.doInsuranceTypeList(mId, mAuthorization,mStart);
            return result;
        }
            ...
            try {
                ...
                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        ...
                        JSONArray feedArray = jsonObj.getJSONArray("insurance_type_list");
                        for (int i = 0; i < feedArray.length(); i++) {

                            ListItem item = new ListItem();
                            JSONObject feedObj = (JSONObject) feedArray.get(i);

                            if((!feedObj.getString("insurance_type").equals(""))
                                    && (!feedObj.getString("insurance_type").equals("null"))) {

                                item.set_id(feedObj.getString("insurance_id"));
                                item.set_name(StringUtils.capitalize(feedObj.getString("insurance_type")
                                        .toLowerCase().trim()));
                                dataItem.add(item);
                            }
                        }
