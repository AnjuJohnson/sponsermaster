...
import com.cutesys.sponsormasterfullversionnew.ADDModule.AddInsuranceCompanyActivity;
...
public class InsuranceCompanyFragment extends Fragment implements View.OnClickListener{
    ...
    private void InitIdView(View rootView){
        ...
        mTitle.setText("Insurance Company List");
        msubtext.setText("Insurance Company");
        InitGetData(false);
    }
    ...
    private void InitGetData(boolean temp){
        ...
        if(mConfig.isOnline(getActivity())){
            LoadInsuranceCompanyInitiate mLoadInsuranceCompanyInitiate = new LoadInsuranceCompanyInitiate(sPreferences.getString("ID", ""),
                    sPreferences.getString("AUTHORIZATION", ""), temp, String.valueOf(start));
            mLoadInsuranceCompanyInitiate.execute((Void) null);
        }else {
    ...
    public void onClick(View view) {
        ...
        switch (buttonId){
            ...
            case R.id.empty_label_retry:
                ...
                break;
            case R.id.mAdd:
                Intent intent = new Intent(getActivity(),AddInsuranceCompanyActivity.class);
    ...
    public class LoadInsuranceCompanyInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {

        private String mId, mAuthorization, mStart;
        private boolean mStatus;

        LoadInsuranceCompanyInitiate(String id, String authorization, boolean status, String Start) {
            mId = id;
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = httpOperations.doInsuranceCompanyList(mId, mAuthorization,mStart);
            return result;
        }
            ...
            try {
                ...
                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        ...
                        JSONArray feedArray = jsonObj.getJSONArray("insurance_company_list");
                        for (int i = 0; i < feedArray.length(); i++) {

                            ListItem item = new ListItem();
                            JSONObject feedObj = (JSONObject) feedArray.get(i);

                            if((!feedObj.getString("insurance_company_name").equals(""))
                                    && (!feedObj.getString("insurance_company_name").equals("null"))) {

                                item.set_id(feedObj.getString("insurance_company_id"));
                                item.set_name(StringUtils.capitalize(feedObj.getString("insurance_company_name")
                                        .toLowerCase().trim()));
                                dataItem.add(item);
                            }
                        }
