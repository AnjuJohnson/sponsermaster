...
import com.cutesys.sponsormasterfullversionnew.ADDModule.AddAllowanceActivity;
import com.cutesys.sponsormasterfullversionnew.ADDModule.AddManufacturerActivity;
import com.cutesys.sponsormasterfullversionnew.ADDModule.AddReasonActivity;
...
public class LeaveReasonFragment extends Fragment implements View.OnClickListener{
    ...
    private void InitIdView(View rootView){
        ...
        mTitle.setText("Reason List");
        msubtext.setText("Reason");
    ...
    private void InitGetData(boolean temp){
        ...
        if(mConfig.isOnline(getActivity())){
            LoadReasonInitiate mLoadReasonInitiate = new  LoadReasonInitiate(sPreferences.getString("ID", ""),
                    sPreferences.getString("AUTHORIZATION", ""), temp, String.valueOf(start));
            mLoadReasonInitiate.execute((Void) null);
        }else {
    ...
    public void onClick(View view) {
        ...
        switch (buttonId){
            ...
            case R.id.empty_label_retry:
                ...
                break;
            case R.id.mAdd:
                Intent intent = new Intent(getActivity(),AddReasonActivity.class);
    ...
    public class  LoadReasonInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {

        private String mId, mAuthorization, mStart;
        private boolean mStatus;

        LoadReasonInitiate(String id, String authorization, boolean status, String Start) {
            mId = id;
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = httpOperations.doReasonList(mId, mAuthorization,mStart);
            return result;
        }
        ...
        protected void onPostExecute(StringBuilder result) {
            ...
            try {
              //JSONArray jsonfeedarray = new JSONArray(result.toString());
                JSONObject jsonObj = new JSONObject(result.toString());

                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        switcher.showContentView();
                        JSONArray feedArray = jsonObj.getJSONArray("leave_reason_list");
                        for (int i = 0; i < feedArray.length(); i++) {

                            ListItem item = new ListItem();
                            JSONObject feedObj = (JSONObject) feedArray.get(i);

                            if((!feedObj.getString("reason").equals(""))
                                    && (!feedObj.getString("reason").equals("null"))) {

                                item.set_id(feedObj.getString("id"));
                                item.set_name(StringUtils.capitalize(feedObj.getString("reason")
                                        .toLowerCase().trim()));
                                dataItem.add(item);
                            }
                        }
