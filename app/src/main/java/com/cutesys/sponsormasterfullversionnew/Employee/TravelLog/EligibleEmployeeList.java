...
public class EligibleEmployeeList extends Fragment implements View.OnClickListener {
   ...
   // TextView mTitile;
    ...
    private void InitIdView(View rootView) {
       ...
       /* mTitile=(TextView)rootView.findViewById(R.id.mTitle);
        mTitile.setText("Eligible Employee List");*/
    ...
    private void InitGetData(boolean temp) {
        ...
        if (mConfig.isOnline(getActivity())) {
            LoadEligibleEmployeeList mLoadEligibleEmployeeList = new LoadEligibleEmployeeList(sPreferences.getString("ID", ""),
                    sPreferences.getString("AUTHORIZATION", ""),String.valueOf(start));
            mLoadEligibleEmployeeList.execute((Void) null);
        } else {
    ...
    public class LoadEligibleEmployeeList extends AsyncTask<Void, StringBuilder, StringBuilder> {
        private String mId, mAuthorization, mStart;
        private boolean mStatus;

        LoadEligibleEmployeeList(String id, String authorization, String Start) {
            mId = id;
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = httpOperations.doEligibleEmployeeList(mId, mAuthorization,mStart);
            return result;
        }
            ...
            try {
                ...
                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        ...
                        JSONArray feedArray = jsonObj.getJSONArray("eligible_employee_list");
                        for (int i = 0; i < feedArray.length(); i++) {
                            ...
                            if ((!feedObj.getString("employee_name").equals(""))
                                ...
                                if (feedObj.getString("employee_designation").trim().equals("")) {
                                    ...
                                    item.setAssigned_employee(feedObj.getString("employee_designation"));
                                }
                                // item.setAssigned_employee(feedObj.getString("employee_designation"));
                                ...
                                // item.setCompany_name(feedObj.getString("company_name"));
                                if (feedObj.getString("no:of leaves").trim().equals("")) {
                                    item.setcompany_address("None");
                                } else {
                                    item.setcompany_address(feedObj.getString("no:of leaves"));
                                }
                                // item.setcompany_address(feedObj.getString("no:of leaves"));
                                dataItem.add(item);
                            }
                        ...
                        // start = dataItem.size();
                        mEmpTravelledAdapter = new EmpTravelledAdapter(getActivity(),getActivity(),dataItem,"eligible");
