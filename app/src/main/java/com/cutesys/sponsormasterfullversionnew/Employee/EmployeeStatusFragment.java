...
public class EmployeeStatusFragment extends Fragment implements View.OnClickListener{
    ...
    private void InitIdView(View rootView){
        ...
        mTitle.setText("Employee Document Status");
        InitGetData(false);
    }
    ...
    private void InitGetData(boolean temp){
        ...
        if(mConfig.isOnline(getActivity())){
            LoadEmpyStatusInitiate mLoadEmpyStatusInitiate = new LoadEmpyStatusInitiate(sPreferences.getString("ID", ""),
                    sPreferences.getString("AUTHORIZATION", ""), temp, String.valueOf(start));
            mLoadEmpyStatusInitiate.execute((Void) null);
        }else {
    ...
    public class LoadEmpyStatusInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {

        private String mId, mAuthorization, mStart;
        private boolean mStatus;

        LoadEmpyStatusInitiate(String id, String authorization, boolean status, String Start) {
            mId = id;
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = httpOperations.doEMPY_DOC_STATUS_LIST(mId, mAuthorization,mStart);
            return result;
        }
            ...
            try {
                ...
                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        ...
                        JSONArray feedArray = jsonObj.getJSONArray("employee_doc_status_list");
                        try {
                            if (feedArray.length() > 0) {
                                for (int i = 0; i < feedArray.length(); i++) {
                                    ...
                                    if((!feedObj.getString("employee_name").equals(""))
                                        ...
                                        item.set_employee_code(feedObj.getString("employee_code"));
                                        item.set_name(StringUtils.capitalize(feedObj.getString("employee_name")
                                                .toLowerCase().trim()));
                                        item.set_employee_visatype(StringUtils.capitalize(feedObj.getString("employee_visatype")
                                                .toLowerCase().trim()));

                                        JSONObject statusObj = new JSONObject(feedObj.getString("Document_Status"));

                                        item.set_address(statusObj.getString("Profile_Picture")
                                                +","+statusObj.getString("Passport"));
                                        item.set_code(statusObj.getString("Resume")
                                                +","+statusObj.getString("Health_Card"));
                                        item.set_model(statusObj.getString("ID")
                                                +","+statusObj.getString("Insurance"));
                                        item.set_manufacture(statusObj.getString("Visa"));
                                        item.set_email(statusObj.getString("License"));
