...
public class AllowanceFragment extends Fragment implements View.OnClickListener{
    ...
    private void InitIdView(View rootView){
        ...
        mTitle.setText("Allowance List");
        msubtext.setText("Allowance");
    ...
    public class LoadManufacturerInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = httpOperations.doAllowanceList(mId, mAuthorization,mStart);
            return result;
        }
            ...
            try {
                ...
                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        ...
                        JSONArray feedArray = jsonObj.getJSONArray("allowance_category_list");
                        for (int i = 0; i < feedArray.length(); i++) {

                            ListItem item = new ListItem();
                            JSONObject feedObj = (JSONObject) feedArray.get(i);

                            if((!feedObj.getString("allowance_name").equals(""))
                                    && (!feedObj.getString("allowance_name").equals("null"))) {

                                item.set_id(feedObj.getString("allowance_id"));
                                item.set_name(StringUtils.capitalize(feedObj.getString("allowance_name")
                                        .toLowerCase().trim()));
                                dataItem.add(item);
                            }
                        }
