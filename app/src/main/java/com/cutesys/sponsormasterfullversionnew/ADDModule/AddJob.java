...
public class AddJob extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.addjobposition);
    ...
    private void InitIdView() {
        ...
        mTitle.setText("Add Job Position");
    }

    private void AddManufacturer() {
        Config mConfig = new Config(getApplicationContext());
        if (mConfig.isOnline(getApplicationContext())) {
            AddJob.LoadAddManufacturerInitiate mLoadAddManufacturerInitiate = new AddJob.LoadAddManufacturerInitiate(sPreferences.getString("ID", ""),
    ...
    private void Close_view(){
        Intent intent = new Intent(AddJob.this,DashboardActivity.class);
    ...
    public class LoadAddManufacturerInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = httpOperations.doAddJobposition(mId, mAuthorization, mdesignation);
            return result;
        }
            ...
            try {
                ...
                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(404))) {
                        ...
                        CustomToast.info(getApplicationContext(), "Job  already exist").show();

                    } else if (jsonObj.getString("status").equals(String.valueOf(200))) {

                        addManufacturer.setText("");
                        CustomToast.success(getApplicationContext(), "Job Added Successfully").show();
                        // close_reveal();
                    }
                }
