...
public class AddVisaClientActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.addvisaclient);
    ...
    private void InitIdView(){
        ...
        mTitle.setText("Add Visa Client");
    }

    private void AddClient(){
        Config mConfig = new Config(getApplication());
        if(mConfig.isOnline(getApplication())){
            LoadAddClientInitiate mLoadAddClientInitiate = new LoadAddClientInitiate(sPreferences.getString("ID", ""),
                    sPreferences.getString("AUTHORIZATION", ""),
                    name.getText().toString().trim(),
                    email.getText().toString().trim());
            mLoadAddClientInitiate.execute((Void) null);
        }else {
    ...
    public class LoadAddClientInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {

        private String mId, mAuthorization, mname, mmail;

        LoadAddClientInitiate(String id, String authorization, String name, String mail) {
            mId = id;
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = httpOperations.doAddClient(mId, mAuthorization, mname,
                    mmail);
            return result;
        }
                ...
                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(404))) {
                        ...
                        if(jsonObj.getString("message").equals("Email Invalid")){
                            ...
                            name.setText("");
                            email.setText("");
                            CustomToast.info(getApplication(), "Client Already Exist").show();
                        }
                        ...
                        name.setText("");
                        email.setText("");
                        CustomToast.success(getApplication(),"Client Added Successfully").show();
                    }
                }
    ...
    private void Close_view(){
        Intent intent = new Intent(AddVisaClientActivity.this,DashboardActivity.class);
        intent.putExtra("PAGE","VISACLIENT");
