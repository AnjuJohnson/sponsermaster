...
public class AddModelActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    private void InitIdView(){
        ...
        mTitle.setText("Add Model");

        Config mConfig = new Config(getApplicationContext());
        if(mConfig.isOnline(getApplicationContext())){
            LoadManufacturerListInitiate mLoadManufacturerListInitiate = new LoadManufacturerListInitiate(sPreferences.getString("ID", ""),
        ...
        spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                mSpinnerItem = dataItem.get(i).get_id();
            }
    ...
    public void onClick(View view) {
        ...
        switch (buttonId) {
            ...
            case R.id.progressrestart:
                Config mConfig = new Config(getApplicationContext());
                if(mConfig.isOnline(getApplicationContext())){
                    LoadManufacturerListInitiate mLoadManufacturerListInitiate =
                            new LoadManufacturerListInitiate(sPreferences.getString("ID", ""),
                                    sPreferences.getString("AUTHORIZATION", ""), String.valueOf(0));
                    mLoadManufacturerListInitiate.execute((Void) null);
                }else {
                    CustomToast.error(getApplicationContext(),"No Internet Connection.").show();
    ...
    public class LoadManufacturerListInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {

        private String mId, mAuthorization, mStart;

        LoadManufacturerListInitiate(String id, String authorization, String Start) {
            mId = id;
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = httpOperations.doManufacturerList(mId, mAuthorization,mStart);
            return result;
        }
            ...
            try {
                ...
                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        ...
                        JSONArray feedArray = jsonObj.getJSONArray("manufacturer_list");
                        for (int i = 0; i < feedArray.length(); i++) {

                            ListItem item = new ListItem();
                            JSONObject feedObj = (JSONObject) feedArray.get(i);

                            if ((!feedObj.getString("manufacturer_name").equals(""))
                                    && (!feedObj.getString("manufacturer_name").equals("null"))) {


                                item.set_id(feedObj.getString("manufacturer_id"));
                                item.set_name(StringUtils.capitalize(feedObj.getString("manufacturer_name")
                                        .toLowerCase().trim()));
    ...
    public class LoadAddModelInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            HttpOperations httpOperations = new HttpOperations(getApplicationContext());
            StringBuilder result = httpOperations.doAddModel(mId, mAuthorization,
                    data, mvid);
            return result;
        }
            ...
            try {
                ...
                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(404))) {
                        ...
                        category.setText("");
                        CustomToast.info(getApplicationContext(), "Model Already exist").show();

                    } else if (jsonObj.getString("status").equals(String.valueOf(200))) {

                        category.setText("");
                        CustomToast.success(getApplicationContext(), "Model Added Successfully").show();
                    }
                }
    ...
    private void Close_view(){
        Intent intent = new Intent(AddModelActivity.this,DashboardActivity.class);
        intent.putExtra("PAGE","MODEL");
