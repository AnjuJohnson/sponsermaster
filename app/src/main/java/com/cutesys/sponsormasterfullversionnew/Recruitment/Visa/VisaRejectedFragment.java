...
import com.cutesys.sponsormasterfullversionnew.Adapterclasses.VisaRejectedAdapter;
import com.cutesys.sponsormasterfullversionnew.Helperclasses.RecruitmentListItem;
import com.cutesys.sponsormasterfullversionnew.R;
//import com.cutesys.sponsormasterfullversionnew.SearchActivity;
...
public class VisaRejectedFragment  extends Fragment implements View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mAdapter = new VisaRejectedAdapter(dataItem);
    ...
    private void InitGetData(boolean temp){
        ...
        if(mConfig.isOnline(getActivity())){
            LoadVisaRejectedInitiate mLoadVisaRejectedInitiate = new LoadVisaRejectedInitiate(sPreferences.getString("ID", ""),
                    sPreferences.getString("AUTHORIZATION", ""), temp, String.valueOf(start));
            mLoadVisaRejectedInitiate.execute((Void) null);
        }else {
    ...
    public class LoadVisaRejectedInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {

        private String mId, mAuthorization, mStart;
        private boolean mStatus;

        LoadVisaRejectedInitiate(String id, String authorization, boolean status, String Start) {
            mId = id;
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = web.doVisaRejectedList(mId, mAuthorization,mStart);
            ...
            try {
                ...
                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        ...
                        JSONArray feedArray = jsonObj.getJSONArray("visa_rejected_list");
                        for (int i = 0; i < feedArray.length(); i++) {
                            ...
                            if ((!feedObj.getString("fullname").equals(""))
                                ...
                                if (feedObj.getString("reason").trim().equals("")) {
                                    item.setreason("None");
                                } else {
                                    item.setreason(StringUtils.capitalize(feedObj.getString("reason")
                                            .toLowerCase().trim()));
                                }
                        ...
                        start = dataItem.size();
                        mAdapter = new VisaRejectedAdapter(getActivity(), dataItem, "cpy");
                        cmpy_recycler9.setAdapter(mAdapter);
                        System.out.println(result);
                    } else {
