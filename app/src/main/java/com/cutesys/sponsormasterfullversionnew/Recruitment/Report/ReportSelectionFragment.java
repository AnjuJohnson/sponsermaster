...
public class ReportSelectionFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        myOnClickListener = new ReportSelectionFragment.MyOnClickListener(getActivity());
        InitIdView(rootView);
        return rootView;
    }
    ...
    private void InitGetData(boolean temp) {
        ...
        if(mConfig.isOnline(getActivity())){
            ReportSelectionFragment.LoadSelectionStatus mLoadVisaprocessreportInitiate= new ReportSelectionFragment.LoadSelectionStatus(sPreferences.getString("ID", ""),
...
//
//    }
    public class  LoadSelectionStatus extends AsyncTask<Void, StringBuilder, StringBuilder> {

        private String mId, mAuthorization, mStart;
        private boolean mStatus;

        LoadSelectionStatus(String id, String authorization, boolean status, String Start) {
            mId = id;
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = web.doReportSelectionStatus(mId, mAuthorization,mStart);
            ...
            try {
                ...
                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        ...
                        JSONArray feedArray = jsonObj.getJSONArray("report_candidate_selected");
                        for (int i = 0; i < feedArray.length(); i++) {
                            ...
                            if ((!feedObj.getString("fullname").equals(""))
                                ...
                                if (feedObj.getString("candidate_interview_status").trim().equals("")) {
                                    item.setvisa_status("None");
                                } else {
                                    item.setvisa_status(StringUtils.capitalize(feedObj.getString("candidate_interview_status")
                                            .toLowerCase().trim()));
                                }
                        ...
                        start = data.size();
                        mVisaProcessAdapter = new VisaProcessAdapter(getActivity(), data,"reportselection");
