...
import com.cutesys.sponsormasterfullversionnew.Adapterclasses.CandidateTravelledAdapter;
...
public class TravelledCandidateFragment  extends Fragment implements View.OnClickListener {
    ...
    CandidateTravelledAdapter mAdapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mAdapter = new CandidateTravelledAdapter(dataItem);
    ...
    private void InitGetData(boolean temp){
        ...
        if(mConfig.isOnline(getActivity())){
            LoadTravelledcandidateInitiate mLoadTravelledcandidateInitiate = new LoadTravelledcandidateInitiate(sPreferences.getString("ID", ""),
                    sPreferences.getString("AUTHORIZATION", ""), temp, String.valueOf(start));
            mLoadTravelledcandidateInitiate.execute((Void) null);
        }else {
    ...
    public class LoadTravelledcandidateInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {

        private String mId, mAuthorization, mStart;
        private boolean mStatus;

        LoadTravelledcandidateInitiate(String id, String authorization, boolean status, String Start) {
            mId = id;
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = web.doTravelledcandidateList(mId, mAuthorization,mStart);
            ...
            try {
                ...
                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        ...
                        JSONArray feedArray = jsonObj.getJSONArray("travelled_candidate_list");
                        for (int i = 0; i < feedArray.length(); i++) {
                            ...
                            if ((!feedObj.getString("fullname").equals(""))
                                ...
                                if (feedObj.getString("entry_date").trim().equals("")) {
                                    item.setentry_date("None");
                                } else {
                                    item.setentry_date((feedObj.getString("entry_date")
                                            .toLowerCase().trim()));
                                }
                                ...
                                if (feedObj.getString("medical_status").trim().equals("")) {
                                    item.setmedical_status("None");
                                } else {
                                    item.setmedical_status(StringUtils.capitalize(feedObj.getString("medical_status")
                                            .toLowerCase().trim()));
                                }
                        ...
                        start = dataItem.size();
                        mAdapter = new CandidateTravelledAdapter(getActivity(), dataItem, "cpy");
                        cmpy_recycler9.setAdapter(mAdapter);
                        System.out.println(result);
                    } else {
