...
import com.cutesys.sponsormasterfullversionnew.ADDModule.AddCountry;
import com.cutesys.sponsormasterfullversionnew.ADDModule.AddQualification;
import com.cutesys.sponsormasterfullversionnew.Adapterclasses.ListHeadingOneAdapter;
import com.cutesys.sponsormasterfullversionnew.Adapterclasses.RecruitSettingsAdapter;
...
public class QualificationFragment extends Fragment implements View.OnClickListener {
    ...
    private void InitIdView(View rootView){
        ...
        mTitle.setText("Qualification List");
        msubtext.setText("Qualification");
    ...
    private void InitGetData(boolean temp){
        ...
        if(mConfig.isOnline(getActivity())){
            QualificationFragment.LoadManufacturerInitiate mLoadManufacturerInitiate = new QualificationFragment.LoadManufacturerInitiate(sPreferences.getString("ID", ""),
                    sPreferences.getString("AUTHORIZATION", ""), temp, String.valueOf(start));
            mLoadManufacturerInitiate.execute((Void) null);
        }else {
    ...
    public class LoadManufacturerInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {

        private String mId, mAuthorization, mStart;
        private boolean mStatus;

        LoadManufacturerInitiate(String id, String authorization, boolean status, String Start) {
            mId = id;
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = httpOperations.dorecruitmentqualificationList(mId, mAuthorization,mStart);
            return result;
        }
            ...
            try {
                ...
                if (jsonObj.has("status")) {
                    if (jsonObj.getString("status").equals(String.valueOf(200))) {
                        ...
                        JSONArray feedArray = jsonObj.getJSONArray("qualification_list");
                        for (int i = 0; i < feedArray.length(); i++) {

                            ListItem item = new ListItem();
                            JSONObject feedObj = (JSONObject) feedArray.get(i);

                            if((!feedObj.getString("qualification_name").equals(""))
                                    && (!feedObj.getString("qualification_name").equals("null"))) {

                                item.set_id(feedObj.getString("qualification_id"));
                                item.set_name(StringUtils.capitalize(feedObj.getString("qualification_name")
                                        .toLowerCase().trim()));
                                dataItem.add(item);
                            }
                        }
