...
public class CompanyReportFragment extends Fragment implements View.OnClickListener{
    ...
    private void InitGetData(boolean temp){
        ...
        if(mConfig.isOnline(getActivity())){
            LoadCompyReportInitiate mLoadCompyReportInitiate = new LoadCompyReportInitiate(sPreferences.getString("ID", ""),
                    sPreferences.getString("AUTHORIZATION", ""), temp, String.valueOf(start));
            mLoadCompyReportInitiate.execute((Void) null);
        }else {
    ...
    public class LoadCompyReportInitiate extends AsyncTask<Void, StringBuilder, StringBuilder> {

        private String mId, mAuthorization, mStart;
        private boolean mStatus;

        LoadCompyReportInitiate(String id, String authorization, boolean status, String Start) {
            mId = id;
        ...
        protected StringBuilder doInBackground(Void... params) {
            ...
            StringBuilder result = httpOperations.doCompanyReport(mId, mAuthorization,mStart);
            return result;
        }
                        ...
                        for (int i = 0; i < feedArray.length(); i++) {
                            ...
                            if((!feedObj.getString("company_name").equals(""))
                                ...
                                if(feedObj.getString("company_pid").trim().equals("")){
                                    ...
                                    item.set_pid(feedObj.getString("company_pid"));
                                }
                               if(feedObj.getString("company_address").trim().equals("")){
                                    item.set_address("None");
                                }else {
                                    item.set_address(feedObj.getString("company_address"));
                                }
                                if(feedObj.getString("company_email").trim().equals("")){
                                    item.set_email("None");
                                }else {
                                    item.set_email(feedObj.getString("company_email"));
                                }

                                if(feedObj.getString("company_status").trim().equals("")){
                                    item.set_phone("None");
                                }else {
                                    item.set_phone(feedObj.getString("company_status"));
                                }
                                dataItem.add(item);
                            }
                        }
